...
public final class KafkaMonitorImpl implements KafkaMonitor {
    ...
    private static class TopicProcessor implements Callable {
        private int startIndex;
        private List<TopicVO> topicInfos;
        private Map<String, Config> topicConfigs;

        TopicProcessor(List<TopicVO> topicInfos, Map<String, Config> topicConfigs) {
    ...
    private Map<String, TopicVO> getTopicMetadata(Map<String, List<PartitionInfo>> allTopicsMap, String... topics) {
        ...
        List<List<TopicVO>> topicInfosSubsets = Lists.partition(topicInfos.values().stream().collect(Collectors.toList()), 4);

        for (var topicInfosSubset : topicInfosSubsets) {
            TopicProcessor task = new TopicProcessor(topicInfosSubset, topicConfigs);
            Future<?> future = executor.submit(task);
            futures.add(future);
        }

        boolean allTasksAreDone = false;

        while (!allTasksAreDone) {
            Set uniqFutures = futures.stream().map(f -> f.isDone()).collect(Collectors.toSet());

            if (uniqFutures.size() == 1 && uniqFutures.contains(true)) {
